...
# --- Configuration ---
TOKEN = '8067690375:AAHVLHEc0XapIcdXGjoyQ-pYy32SQ-kK40E'
OWNER_ID = 6288814828
ADMIN_ID = 6288814828
YOUR_USERNAME = '@onlysiyam99'
UPDATE_CHANNEL = 'https://t.me/onlysiyam99'

UPDATE_CHANNEL_USERNAME = 'onlysiyam99'
REQUIRED_GROUP_USERNAME = 'FFUIDALL'
CHANNEL_ID = None
GROUP_ID = None
...
# --- Group/Channel Chat Check Decorator ---
def private_chat_only(func):
    """Decorator to ensure bot only responds in private chats"""
    def wrapper(message, *args, **kwargs):
        chat_type = message.chat.type
        if chat_type in ['group', 'supergroup', 'channel']:
            logger.info(f"Ignoring message from {chat_type}: {message.chat.id}. User: {message.from_user.id}")
            return
...
def check_membership(user_id):
    """Check if user is member of required channel and group (only in private)"""
    try:
        channel_status = bot.get_chat_member(f"@{UPDATE_CHANNEL_USERNAME}", user_id)
        if channel_status.status in ['left', 'kicked']:
            return False, "channel"
        
        group_status = bot.get_chat_member(f"@{REQUIRED_GROUP_USERNAME}", user_id)
        if group_status.status in ['left', 'kicked']:
            return False, "group"
...
def require_membership(func):
    """Decorator to require channel and group membership before executing command (private only)"""
    def wrapper(message, *args, **kwargs):
        ...
        if not is_member:
            if missing == "channel":
                ...
                markup.add(types.InlineKeyboardButton('📢 Join Channel', url=f'https://t.me/{UPDATE_CHANNEL_USERNAME}'))
                markup.add(types.InlineKeyboardButton('✅ I Have Joined', callback_data='check_after_join'))
                bot.reply_to(message, "⚠️ You must join our channel to use this bot!", reply_markup=markup)
                return
            elif missing == "group":
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton('👥 Join Group', url=f'https://t.me/{REQUIRED_GROUP_USERNAME}'))
                markup.add(types.InlineKeyboardButton('✅ I Have Joined', callback_data='check_after_join'))
                bot.reply_to(message, "⚠️ You must join our group to use this bot!", reply_markup=markup)
                return
            else:
                markup = types.InlineKeyboardMarkup()
                markup.row(types.InlineKeyboardButton('📢 Join Channel', url=f'https://t.me/{UPDATE_CHANNEL_USERNAME}'))
                markup.row(types.InlineKeyboardButton('👥 Join Group', url=f'https://t.me/{REQUIRED_GROUP_USERNAME}'))
                markup.add(types.InlineKeyboardButton('✅ I Have Joined', callback_data='check_after_join'))
                bot.reply_to(message, "⚠️ You must join our channel and group to use this bot!", reply_markup=markup)
                return
...
def create_join_required_markup():
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton('📢 Join Channel', url=f'https://t.me/{UPDATE_CHANNEL_USERNAME}'))
    markup.row(types.InlineKeyboardButton('👥 Join Group', url=f'https://t.me/{REQUIRED_GROUP_USERNAME}'))
    markup.add(types.InlineKeyboardButton('✅ I Have Joined', callback_data='check_after_join'))
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    temp_dir = None
    try:
        temp_dir = tempfile.mkdtemp(prefix=f"user_{user_id}_zip")
...
def _logic_send_welcome(message):
    ...
    # Check membership for non-admin users (only in private chat)
    if user_id not in admin_ids:
        is_member, missing = check_membership(user_id)
        if not is_member:
            if missing == "channel":
                bot.send_message(chat_id, "⚠️ You must join our channel to use this bot!", reply_markup=create_join_required_markup())
                return
            elif missing == "group":
                bot.send_message(chat_id, "⚠️ You must join our group to use this bot!", reply_markup=create_join_required_markup())
                return
            else:
                bot.send_message(chat_id, "⚠️ You must join our channel and group to use this bot!", reply_markup=create_join_required_markup())
                return
...
def _logic_upload_file(message):
    ...
    if user_id not in admin_ids:
        ...
        if not is_member:
            if missing == "channel":
                bot.reply_to(message, "⚠️ You must join our channel to use this bot!", reply_markup=create_join_required_markup())
                return
            elif missing == "group":
                bot.reply_to(message, "⚠️ You must join our group to use this bot!", reply_markup=create_join_required_markup())
                return
            else:
                bot.reply_to(message, "⚠️ You must join our channel and group to use this bot!", reply_markup=create_join_required_markup())
                return
...
def handle_callbacks(call):
    ...
    if call.message.chat.type in ['group', 'supergroup', 'channel']:
        logger.info(f"Ignoring callback from {call.message.chat.type}: {call.message.chat.id}. User: {user_id}")
        return

    if user_id not in admin_ids and data not in ['check_after_join', 'back_to_main', 'speed', 'stats']:
        is_member, missing = check_membership(user_id)
        if not is_member:
            if missing == "channel":
                bot.answer_callback_query(call.id, "⚠️ Join the channel!", show_alert=True)
                bot.edit_message_text("⚠️ You must join our channel to use this bot!", 
                                    call.message.chat.id, call.message.message_id, 
                                    reply_markup=create_join_required_markup())
                return
            elif missing == "group":
                bot.answer_callback_query(call.id, "⚠️ Join the group!", show_alert=True)
                bot.edit_message_text("⚠️ You must join our group to use this bot!", 
                                    call.message.chat.id, call.message.message_id, 
                                    reply_markup=create_join_required_markup())
                return
            else:
                bot.answer_callback_query(call.id, "⚠️ Join channel and group!", show_alert=True)
                bot.edit_message_text("⚠️ You must join our channel and group to use this bot!", 
                                    call.message.chat.id, call.message.message_id, 
                                    reply_markup=create_join_required_markup())
                return

    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'check_after_join']:
        bot.answer_callback_query(call.id, "⚠️ Bot locked by admin.", show_alert=True)
        return

    try:
        if data == 'check_after_join':
            is_member, missing = check_membership(user_id)
            if is_member:
                ...
                if missing == "channel":
                    ...
                    bot.answer_callback_query(call.id, "❌ You still haven't joined the group!", show_alert=True)
                else:
                    bot.answer_callback_query(call.id, "❌ You still haven't joined channel and group!", show_alert=True)
        elif data == 'upload':
...
if __name__ == '__main__':
    ...
    logger.info(f"📢 Required Channel: @{UPDATE_CHANNEL_USERNAME}")
    logger.info(f"👥 Required Group: @{REQUIRED_GROUP_USERNAME}")
    keep_alive()
