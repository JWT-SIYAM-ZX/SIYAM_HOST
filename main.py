...
# --- PELLA.APP COMPATIBLE KEEP ALIVE ---
# This will start a small Flask app on the provided PORT (or 8080 by default)
# so Pella.app or other platform health checks can reach the process.
from flask import Flask
from threading import Thread
import os as __os_for_pella

_app = Flask(__name__)

@_app.route('/')
def _home():
    return "Bot is running (pella.app compatible)."

def _run_flask():
    port = int(__os_for_pella.environ.get("PORT", 8080))
...
# ensure keep_alive() is called in the startup section below
# --- END KEEP ALIVE ---
